...
const Developer = () => {
  return (
    <div className="bg-black pt-20 pb-30" id="Developer">
      <div className="max-w-6xl mx-auto text-center mb-12">
        <p className="text-gray-400 text-sm font-medium">
          Trusted by the world's most innovative teams
        </p>
      </div>
      <div className="max-w-5xl mx-auto grid grid-cols-4 border-white gap-3">
        {companies.map((company) => (
          <div
            key={company.id}
            className="flex items-center justify-center p-9 border border-white/20 rounded-xl "
          >
            <img src={company.src} alt={company.name} className="w-30 h-9" />
          </div>
        ))}
      </div>
    </div>
